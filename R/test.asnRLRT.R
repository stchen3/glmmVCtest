#' Asymptotic-normalized RLRT for variance components in GLMMs
#'
#' Conducts an asymptotic-normalized restricted-likelihood ratio test (as-nRLRT)
...
#'
#' Compared to the recommended nRLRT, this method instead uses an asymptotic null
...
#' \itemize{
#' \item \code{asnRLRT}: outcome of the asnRLRT test (statistic and p-value)
...
#' asnRLRT <- test.asnRLRT(glmm.fit)
#' }
#'
#' @seealso
#' \code{\link{test.nRLRT}}

test.asnRLRT<-function(fit.glmmPQL,nsim=10000){
  ...
  asnRLRT.stat <- max(0,as.numeric(-2*(logLik(fit.null,REML=T)-logLik(fit.alt,REML=T))))
  n.fixed.diff <- n.fix.alt - n.fix.alt
  null.dist <- asymptotic.null(n.fixed.diff)
  asnRLRT.p <- mean(asnRLRT.stat<=null.dist)
  asnRLRT <- list(statistic=asnRLRT.stat,p=asnRLRT.p)

  return(list(asnRLRT=asnRLRT,std.data=std.data,fit.alt=fit.alt,fit.null=fit.null))
}
